...
class MotionTransformer(nn.Module):
    ...
    def encode_text(self, text, device):
        with torch.no_grad():
            ...
            if self.training:
              mask = torch.bernoulli(torch.ones(x.shape[:-1])*0.9).to(device)
              x = x * mask[:,:,None]
        ...
        #x = x * (torch.rand(x.shape[:-1])<0.9).float().to(device)[:,:,None]
...
class InteractionMotionTransformer(MotionTransformer):
  def __init__(self,
                 input_feats,
                 num_frames=240,
                 latent_dim=512,
                 ...
                 no_eff=False,
                 **kargs):
        super().__init__(input_feats, num_frames, latent_dim, ff_size, num_layers,
                 num_heads, dropout, activation, num_text_layers, text_latent_dim,
                 text_ff_size, text_num_heads, no_clip, no_eff,**kargs)
        self.temporal_decoder_blocks = nn.ModuleList()
        for i in range(num_layers):
          decoder_block = nn.ModuleList()
          decoder_block.append(
            LinearTemporalSelfAttention(num_frames, latent_dim, num_heads, dropout, self.time_embed_dim)
          )
          decoder_block.append(
            LinearTemporalCrossAttention(num_frames, latent_dim, text_latent_dim, num_heads, dropout, self.time_embed_dim)
          )
          decoder_block.append(
            LinearTemporalCrossAttention(num_frames, latent_dim, latent_dim, num_heads, dropout, self.time_embed_dim)
          )
          decoder_block.append(
            FFN(latent_dim, ff_size, dropout, self.time_embed_dim)
          )
          self.temporal_decoder_blocks.append(decoder_block)
  ...
  def forward(self, x, timesteps, length=None, text=None, xf_proj=None, xf_out=None):
        ...
        x: B, T, D
        """
        x1 = x[:,:,:self.input_feats]
        x2 = x[:,:,self.input_feats:]
        B, T = x1.shape[0], x1.shape[1]
        if text is not None and len(text) != B:
            index = x1.device.index
            text = text[index * B: index * B + B]
        if xf_proj is None or xf_out is None:
            xf_proj, xf_out = self.encode_text(text, x1.device)
        
        emb = self.time_embed(timestep_embedding(timesteps, self.latent_dim)) + xf_proj

        # B, T, latent_dim
        h1 = self.joint_embed(x1)
        h1 = h1 + self.sequence_embedding.unsqueeze(0)[:, :T, :]
        h2 = self.joint_embed(x2)
        h2 = h2 + self.sequence_embedding.unsqueeze(0)[:, :T, :]
        
        
        src_mask = self.generate_src_mask(T, length).to(x1.device).unsqueeze(-1)
        for module in self.temporal_decoder_blocks:
            self_attention = module[0]
            h1 = self_attention(h1, emb, src_mask)
            h2 = self_attention(h2, emb, src_mask)
            text_cross_attention = module[1]
            h1 = text_cross_attention(h1, xf_out, emb)
            h2 = text_cross_attention(h2, xf_out, emb)
            motion_cross_attention = module[2]
            h1_ = motion_cross_attention(h1, h2, emb, src_mask)
            h2_ = motion_cross_attention(h2, h1, emb, src_mask)
            ffn = module[3]
            h1 = ffn(h1_, emb)
            h2 = ffn(h2_, emb)

        output1 = self.out(h1).view(B, T, -1).contiguous()
        output2 = self.out(h2).view(B, T, -1).contiguous()
        output = torch.cat([output1,output2],dim=2).contiguous()
        return output
