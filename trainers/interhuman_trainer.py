...
class InterhumanTrainer(object):
    ...
    def bone_len(self, joints):
        ...
        for chain in t2m_kinematic_chain:
          for i in range(len(chain)-1):
            idx1 = chain[i]
            idx2 = chain[i+1]
            bone = torch.norm(joints[:,:,idx1,:]-joints[:,:,idx2,:],dim=2)[:, :, None]
            out.append(bone)
    ...
    def forward(self, batch_data, eval_mode=False):
        ...
        output = self.diffusion.training_losses(
            ...
            x_start=x_start,
            t=t,
            model_kwargs={"text": text, "length": cur_len}
        )
        ...
        self.joints_real = x_start.view(B, T, 2*self.num_joints, 3)
        self.joints_pred = x_pred.view(B, T, 2*self.num_joints, 3)
        self.t = t
    ...
    def generate(self, caption, m_lens, dim_pose, batch_size=128):
        N = len(caption)
        cur_idx = 0
        self.encoder.eval()
        all_output = []
    ...
    def backward_G(self):
        ...
        joints_pred = (self.joints_pred * self.src_mask[:,:,None,None])[self.t<500]
        
        joints_real = (self.joints_real * self.src_mask[:,:,None,None])[self.t<500]
        
        loss_pos = torch.nn.functional.mse_loss(joints_pred, joints_real)
        velo_pred = joints_pred[:,1:,:,:] - joints_pred[:,:-1,:,:]
        velo_real = joints_real[:,1:,:,:] - joints_real[:,:-1,:,:]
        ...
        self.num_joints+l_ankle_idx, self.num_joints+l_foot_idx, self.num_joints+r_ankle_idx, 
        self.num_joints+r_foot_idx]
        
        fc_joints_pred = joints_pred[:,:,relevant_joints,:]
        fc_joints_real = joints_real[:,:,relevant_joints,:]
        ...
        joints_pred1 = joints_pred[:,:,:self.num_joints,:]
        joints_pred2 = joints_pred[:,:,self.num_joints:,:]
        joints_real1 = joints_real[:,:,:self.num_joints,:]
        joints_real2 = joints_real[:,:,self.num_joints:,:]
        pred_bone_length1 = self.bone_len(joints_pred1)
        pred_bone_length2 = self.bone_len(joints_pred2)
        real_bone_length1 = self.bone_len(joints_real1)
        real_bone_length2 = self.bone_len(joints_real2)
        
        loss_bone = F.mse_loss(pred_bone_length1,real_bone_length1) + F.mse_loss(pred_bone_length2,real_bone_length2)

        self.loss = loss_mot_rec + (loss_pos + 5*loss_velo + loss_fc + loss_bone ) 
        ...
        loss_logs['loss_bone'] = loss_bone.item()
        return loss_logs
    ...
    def load(self, model_dir):
        ...
        return checkpoint.get('ep',0), checkpoint.get('total_it',0)
