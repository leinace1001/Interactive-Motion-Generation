...
class InterhumanGenerateMotion(Dataset):
    def __init__(self, opt, trainer, dataloader):
        all_caption = []
        all_m_lens = []
        all_data = []
        ...
        with torch.no_grad():
            for i, data in tqdm(enumerate(dataloader)):
                _, caption, _, motions, m_lens= data                
                
                all_m_lens.append(m_lens)
                all_caption.extend(caption)
                
        all_m_lens = torch.cat(all_m_lens,dim=0)
        # Generate all sequences
        trainer.eval_mode()
        trainer.to(opt.device)
        with torch.no_grad():
        ...
        for i in range(len(all_pred_motions)):
        
          _,D = all_pred_motions[i].shape
          cur_len = all_m_lens[i].item()
          if(cur_len<opt.max_motion_length):
            
            all_pred_motions[i] = torch.cat([all_pred_motions[i][:cur_len], torch.zeros(opt.max_motion_length-cur_len,D).to(opt.device)],dim=0)


        self.all_m_lens = all_m_lens
        self.all_pred_motions = torch.stack(all_pred_motions)
        self.all_caption = all_caption
        

    def __getitem__(self, index):
        tokens = self.tokenizer(self.all_caption[index], return_tensors="pt")['input_ids']
        ...
        return word_emb, self.all_caption[index], sent_len, self.all_pred_motions[index], self.all_m_lens[index]

    def __len__(self):
        return len(self.all_caption)
...
class MMGeneratedDatasetInterhuman(Dataset):
    def __init__(self, opt, motion_dataloader):
        self.opt = opt
        self.dataset = motion_dataloader
        self.mm_motions = []
        self.m_lens = []
        for i, batch in enumerate(motion_dataloader):
            motion, _, length = batch
            self.mm_motions.append(motion)
            self.m_lens.append(length)

    def __len__(self):
        return len(self.m_lens)

    def __getitem__(self, index):
        motions = self.mm_motions[index]
        lens = self.m_lens[index]
        sort_indx = np.argsort(lens.cpu().numpy())[::-1].copy()
        lens = lens[sort_indx]
...
def build_models(opt):
    ...
    checkpoint = torch.load('checkpoints/interhuman_eval/finest.tar',
...
class EvaluatorModelWrapper(object):
    ...
    def get_co_embeddings(self, word_embs, cap_lens, motions, m_lens):
        with torch.no_grad():
            ...
            '''Text Encoding'''
            text_embedding = self.text_encoder(word_embs, cap_lens)
